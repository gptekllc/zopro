...
const PayPalConnectReturn = () => {
  ...
  useEffect(() => {
    const syncStatus = async () => {
      try {
        // Call the sync function to update company's PayPal status
        const { data, error } = await supabase.functions.invoke('sync-paypal-connect-status');
        
        if (error) {
          console.error('Error syncing PayPal status:', error);
          setSyncError('Failed to verify PayPal account status');
        } else if (data?.status) {
  ...
  const isComplete = status?.onboardingComplete && status?.hasPayPalAccount;
          ...
          <CardHeader className="text-center">
            {isSyncing ? (
              <>
                ...
                <CardDescription>
                  Please wait while we verify your PayPal account status.
                </CardDescription>
              </>
            ) : syncError ? (
              <>
              ...
              <>
                ...
                <CardTitle className="text-2xl">PayPal Setup Complete</CardTitle>
                <CardDescription>
                  Your PayPal account has been connected. You can now accept PayPal payments for invoices.
                </CardDescription>
              </>
            ) : (
              <>
                ...
                <CardDescription>
                  Your PayPal account setup is not complete yet. Please continue the setup process in Company Settings.
                </CardDescription>
              </>
            )}
          </CardHeader>
            ...
            {!isSyncing && isComplete && (
              <div className="bg-muted/50 rounded-lg p-4 text-sm text-muted-foreground">
                ...
                <ul className="list-disc list-inside mt-2 space-y-1">
                  <li>Customers can pay invoices using PayPal</li>
                  <li>Payments are deposited to your PayPal account</li>
...
export default PayPalConnectReturn;
