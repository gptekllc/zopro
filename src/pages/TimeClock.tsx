...
const TimeClockContent = () => {
  ...
  const availableJobs = jobs.filter(j => j.status === 'scheduled' || j.status === 'in_progress' || j.status === 'draft');
  ...
  return <PageContainer width="full" className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Time Clock</h1>
          ...
          <Card className="overflow-hidden relative border-0 shadow-xl bg-gradient-to-br from-background via-background to-muted/30">
            ...
            <div className="p-6 sm:p-8 lg:p-10 flex flex-col items-center justify-center text-center min-h-[400px] lg:min-h-[420px] relative z-10">
              ...
              {(status === 'working' || status === 'break') && <div className="mb-6 sm:mb-8 p-4 bg-muted/30 backdrop-blur-sm rounded-2xl border border-border/50 w-full max-w-xs shadow-inner">
                  ...
                  {activeEntry?.job && <div className="mt-2 text-xs text-muted-foreground flex items-center justify-center gap-1.5 bg-background/50 rounded-full px-3 py-1 w-fit mx-auto">
              ...
              {status === 'idle' && availableJobs.length > 0 && <div className="w-full max-w-sm mb-4 space-y-3">
                  ...
                  <Select value={selectedJobId} onValueChange={setSelectedJobId}>
                    ...
                    <SelectContent>
                      ...
                      {availableJobs.map(job => <SelectItem key={job.id} value={job.id}>
                          {job.job_number} - {job.title}
                        </SelectItem>)}
                    </SelectContent>
                  </Select>
                  
                  {selectedJobId !== 'none' && <div className="flex items-start gap-3 p-3 bg-muted/30 rounded-xl border border-border/50 text-xs backdrop-blur-sm">
                      <Checkbox id="record-work-hours" checked={recordWorkHours} onCheckedChange={checked => setRecordWorkHours(checked === true)} className="mt-0.5" />
              ...
              <div className="divide-y max-h-[250px] overflow-y-auto">
                {userEntries.length === 0 ? <div className="p-8 text-center text-muted-foreground text-sm">No activity yet</div> : userEntries.slice(0, 6).map(entry => <div key={entry.id} className="p-3 flex items-start gap-3 hover:bg-muted/50 transition-colors cursor-pointer" onClick={() => handleViewEntry(entry)}>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        {entry.job && <p className="text-xs text-muted-foreground truncate mt-0.5 flex items-center gap-1">
          ...
          <div className="p-4 border-b flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div className="flex items-center bg-muted rounded-md p-0.5">
                ...
                <button className="p-1 hover:bg-background rounded-sm transition-all" onClick={() => setWeekOffset(prev => prev + 1)} disabled={weekOffset >= 0}>
                  <ChevronRight className="w-4 h-4 text-muted-foreground" />
                </button>
              </div>
            </div>
            {canViewReports && <div className="flex items-center gap-2">
                <Link to="/timesheet">
            ...
            <table className="w-full text-sm text-left">
              ...
              return <tr key={day.toISOString()} className={`hover:bg-muted/30 ${index % 2 === 1 ? 'bg-muted/10' : ''}`}>
                      <td className="px-4 py-3">
                        ...
                        {dayEntries.length > 0 && firstEntry?.job && <div className="text-xs text-muted-foreground">{firstEntry.job.job_number}</div>}
                      </td>
