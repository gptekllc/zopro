...
const AppLayout = ({
  ...
  const handleOnLeaveToggle = async (checked: boolean) => {
    ...
    try {
      ...
      if (checked && profile.company_id) {
        const {
          data: managers
        } = await (supabase as any).from('profiles').select('id, full_name, email').eq('company_id', profile.company_id).in('role', ['admin', 'manager']).neq('id', user.id);
  ...
  const filteredNavItems = navItems.filter(item => item.roles.some(role => userRoles.includes(role as any)) || userRoles.length === 0);
  ...
  const checkCustomerPortalAccess = useCallback(async () => {
    ...
    try {
      const {
        data,
        error
      } = await supabase.functions.invoke('customer-portal-auth', {
        body: {
          action: 'check-auth-user-access'
        }
      });
      if (error) throw error;
  ...
  const handleSwitchToCustomerPortal = async () => {
    try {
      const {
        ...
        body: {
          action: 'create-link-for-auth-user'
        }
      });
      if (error) throw error;
      if (!data?.url) {
  ...
  return <div className="min-h-screen bg-background">
      {/* Offline Indicator */}
      <OfflineIndicator />
      <header className="lg:hidden fixed left-0 right-0 bg-card border-b z-50 px-4" style={{
              ...
              <DropdownMenuContent align="end" className="bg-popover">
                <div className="flex items-center justify-between px-2 py-1.5">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <Switch checked={isOnLeave} onCheckedChange={handleOnLeaveToggle} disabled={isUpdatingStatus} />
                  </div>
                </div>
                ...
                {canSwitchPortals && <>
                    <DropdownMenuSeparator />
                    <DropdownMenuItem disabled={isCheckingCustomerPortalAccess} onClick={isInCustomerPortal ? handleSwitchToCompanyPortal : handleSwitchToCustomerPortal}>
      ...
      <aside className={cn("hidden lg:block fixed top-0 left-0 h-full bg-sidebar text-sidebar-foreground z-50 transition-all duration-300", sidebarCollapsed ? "w-[72px]" : "w-64")}>
        <div className="flex flex-col h-full relative">
          {/* Floating toggle button */}
          <Tooltip>
            <TooltipTrigger asChild>
              <Button variant="outline" size="icon" onClick={toggleSidebar} className="absolute top-4 -right-3 z-10 h-6 w-6 rounded-full border bg-background shadow-md text-muted-foreground hover:bg-accent hover:text-accent-foreground">
          ...
          <button onClick={() => navigate('/dashboard')} className={cn("flex items-center gap-3 shrink-0 hover:opacity-80 transition-opacity text-left", sidebarCollapsed ? "p-3 justify-center" : "p-6")}>
          ...
          <div className="p-3 border-t border-sidebar-border shrink-0">
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <button className={cn("w-full flex items-center gap-3 p-2 rounded-lg bg-sidebar-accent hover:bg-sidebar-accent/80 transition-colors", sidebarCollapsed && "justify-center p-2")}>
      ...
      {/* Main content */}
      <main className={cn("lg:pt-0 pb-20 lg:pb-0 min-h-screen transition-all duration-300", sidebarCollapsed ? "lg:ml-[72px]" : "lg:ml-64")} style={{
      paddingTop: 'calc(var(--safe-area-top) + 4rem)' // 4rem = h-16 header on mobile
    }}>
        <div className={cn('w-full p-3 lg:py-6 lg:px-6', contentWidth === 'contained' && 'mx-auto max-w-7xl')}>
          {children}
        </div>
      </main>
