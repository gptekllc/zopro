...
serve(async (req) => {
  ...
  try {
    ...
    if (!requesterProfile || requesterProfile.role !== "admin") {
      return new Response(JSON.stringify({ error: "Only admins can force sign out users" }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    // Sign out users by invalidating their sessions
    const results = [];
    for (const targetUser of targetUsers) {
      try {
        ...
        // Also revoke their trusted devices
        await supabaseAdmin
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: `${successCount} user(s) signed out from all devices`,
        results 
      }),
      { status: 200, headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error: unknown) {
    console.error("Error in force-signout-user:", error);
