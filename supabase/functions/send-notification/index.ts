...
interface NotificationRequest {
  type: 'company_assigned' | 'roles_changed';
...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const { type, recipientEmail, recipientName, companyName, roles, previousRoles }: NotificationRequest = await req.json();
        ...
        <html>
          <head>
            <style>
              ...
              .badge { display: inline-block; background: #6366f1; color: white; padding: 5px 15px; border-radius: 20px; font-size: 14px; }
        ...
        <html>
          <head>
            <style>
              ...
              .header { background: linear-gradient(135deg, #6366f1 0%, #8b5cf6 100%); color: white; padding: 30px; border-radius: 10px 10px 0 0; }
              .content { background: #f9fafb; padding: 30px; border-radius: 0 0 10px 10px; }
              .role-badge { display: inline-block; background: #6366f1; color: white; padding: 5px 15px; border-radius: 20px; font-size: 14px; margin: 2px; }
              .role-badge.removed { background: #ef4444; text-decoration: line-through; }
              .role-badge.new { background: #22c55e; }
